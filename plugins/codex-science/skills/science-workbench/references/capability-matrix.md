...
| Visual research workspace | Self-contained local HTML portal covering workflow, ledgers, services, provider status, next actions, and capability gaps | ready as a read-only static workspace; not a proprietary native application shell |
| Cross-session continuity | Generated resume capsule, append-oriented lab notes, project files, and provenance-preserving forks | ready for file-based handoff; thread/platform memory remains deployment-dependent |
| Public-feature conformance audit | Machine-readable report generated from a pinned public capability specification and local tool availability | ready for feature transparency; explicitly not model-quality or benchmark parity |
